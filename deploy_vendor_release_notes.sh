...
# file path refering to a vendor release notes
...
do
  case $option in
    h) cat <<- EOF
Usage: deploy_vendor_release_notes.sh [options] <path>

Accepts one argument: <path>, a URL path that references the Vendor
Facing Release Notes project source directory; packages the source
directory; posts the package to an AWS S3 bucket; and sends a
notification email to GSA.

Example
> deploy_vendor_release_notes.sh \
    /home/ubuntu/vendor-facing-release-notes
...
source="vendor-facing-release-notes-$timestamp"
package="$source.zip"
hash="$package.sha1"
